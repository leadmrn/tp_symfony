...
class AppFixtures extends Fixture
{
    public function load(ObjectManager $manager)
    {
        ...
        $repository = $manager->getRepository(Beer::class);
        $repository->findThreeLastBeer();
        ...
        while ($count < 20) {
            ...
            // On récupère toutes les catégories 'normal'
            $categoryNormal = $repoCategory->findBy([
                'term' => 'normal'
            ]);
            // On récupère toutes les catégories 'special'
            $categorySpecial = $repoCategory->findBy([
                'term' => 'special'
            ]);

            //On ajoute une catégorie 'normal' à chaque bière
            $beer->addCategory($categoryNormal[rand(0, count($categoriesNormals) - 1)]);
            //On ajoute une catégorie 'special' à chaque bière
            $beer->addCategory($categorySpecial[rand(0, count($categoriesSpecials) - 1)]);
            //Certaines bières auront plusieurs catégories 'special'
            if(rand(1,2) === 2){
                $otherCategorySpecial = $categorySpecial[rand(0, count($categoriesSpecials) - 1)];
                if($beer->getCategories() === $otherCategorySpecial){
                    return;
                } else {
                    $beer->addCategory($otherCategorySpecial);
                }
            }
